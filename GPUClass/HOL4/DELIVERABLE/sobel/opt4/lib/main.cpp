...
int main(int argc, char* argv[]) {
	...
	if(intensityInput->width()%PITCH_MUL)
    {
        cpu.width  = intensityInput->width()+PITCH_MUL-intensityInput->width()%PITCH_MUL;
    } else
    {
	...
	for (unsigned int y = 0 ; y < YSize ; y++){
		for (unsigned int x = 0 ; x < XSize ; x++){
			...
			if (x < intensityInput->width() && y < intensityInput->height()) {
				cpu.intensity[newLocation] = intensityInput->data[y*intensityInput->width() + x];
			} else{
	...
	// Create output image
	image<Color> output = image<Color>(intensityInput->width(), intensityInput->height(), true);
	image<Color>* im = &output;
	
	Color randomcolor = randomColor();
	for (unsigned int y = 0 ; y < intensityInput->height() ; y++){
		for (unsigned int x = 0 ; x < intensityInput->width() ; x++){
			unsigned int newLocation = imageLocation (x, y, cpu.gridXSize);
